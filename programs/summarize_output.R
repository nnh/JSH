...
#           2025.07.25 Agata.K 2025年度集計：エラーがいくつか発生したため、不安定な箇所を改善
#######################################################################
...
for(i in 1:length(disease_detail)) {
  ...
  # ★★★ disease_detail[[i]] がNULLでない場合のみ、処理を実行 ★★★
  if (!is.null(disease_detail[[i]])) {
    ...
    # 結合したら、id情報は不要になるので、削除（存在する場合のみ）（2025.07.22 Agata.K 動きが不安定なため改良）
    disease_detail[[i]] <- disease_detail[[i]] %>% select(-any_of("id"))
  }
}

# # 結合したら、id不要なので削除を追加（2022.7.28 Agata.K）2025.07.22 Agata.K 削除
...
# 図が不要になったため、以下は削除（2025.07.22 Agata.K）
# #--------------------------------
# # 関数：Common_style
# # 内容：集計表の共通書式を設定する関数
# # 引数：data()
