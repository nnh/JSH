...
date.cutoff <- "20230531" # データ固定日
kYear <- "2022"           # 集計する診断年
flag <- 1                 # WHO2016で集計する場合は1を入力、WHO2008で集計する集計する場合は2を入力
# programを保管しているパス
prtpath <- "C:/Users/KumikoAgata/Box/Datacenter/Trials/JSH/Registry/10.03.10 データレビュー書/2022年診断/集計/1_成形"
kToday <- Sys.Date()

# rawdataフォルダ内のファイル読込（tidyverseパッケージのread_csvを使用）(2022/7/28 Agata.K)
rawdatapath <- paste0(prtpath, "/rawdata/") # DLデータ保管フォルダ
jspho_rgst <- read_csv(paste0(rawdatapath, "JSPHO_registration_230727_0758.csv")) # JSPHOのDLデータ読込
jspho_outcome <- read_csv(paste0(rawdatapath, "JSPHO_220722_1533.csv"))

jsh_report <- read_csv(paste0(rawdatapath, "JSH_report_230703_1044.csv"))         # JSHのDLデータ読込
jsh.rgst <- read_csv(paste0(rawdatapath, "JSH_registration_230703_1044.csv"))
